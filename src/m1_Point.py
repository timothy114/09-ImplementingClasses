...
class Point(object):
    ...
    def move_to(self,x,y):
        self.distance_traveled += Point.get_distance_from(self,Point(x,y))
        self.x = x
        self.y = y
        self.move_count += 1

    def move_by(self,dx,dy):
        self.distance_traveled += Point.get_distance_from(self,Point(self.x + dx,self.y + dy))
        self.x += dx
        self.y += dy
        self.move_count += 1
    ...
    def get_distance_from(self,point_2):
        dist = math.sqrt(((self.x-point_2.x) ** 2) + ((self.y-point_2.y) ** 2))
        return dist

    def get_distance_from_start(self):
        dist = Point.get_distance_from(self,Point(self.start_x,self.start_y))
        return dist

    def get_distance_traveled(self):
        return self.distance_traveled

    def closer_to(self,point_1,point_2):
        dist_1 = Point.get_distance_from(self,point_1)
        dist_2 = Point.get_distance_from(self,point_2)
        if dist_1 <= dist_2:
            return point_1
        else:
            return point_2

    def halfway_to(self,point_1):
        new_x = (point_1.x + self.x)/2
        new_y = (point_1.y + self.y)/2
